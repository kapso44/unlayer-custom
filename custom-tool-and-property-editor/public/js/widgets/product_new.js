$(document).ready(function () {
  var productFeedApi = 'https://bm-feed.targetbay.com/api/v1/woocommerce/product/all';
  var uniqueId = '';
  var productId = 0;
      ...
      mount(node, value, updateValue) {
        $(document).ready(function () {
          ...
          $('#search-' + uniqueId).keyup(function () {
            ...
            $.post(productFeedApi, function (data) {
              $.each(data, function (key, value) {
                let pid = value.product_id.toString();
                if (value.title.search(expression) != -1 || pid.search(searchField) != -1) {
                  const { images, title, product_id, currency_code, price, compare_price } = value;
                  let imageSrc = images ? images['src'] : 'https://unroll-images-production.s3.amazonaws.com/projects/4251/1589964645932-200.png';
                  $('#result-' + uniqueId).append('' +
                    `<li class="link-class list-group-item d-flex justify-content-between align-items-center">
                        <div class="image-parent image-src col-4" data-image="${imageSrc}">
                          <img src="${imageSrc}" height="100" width="100" class="img-thumbnail"/>
                        </div>
                        <div class="details col-8">
                          <span class="name" data-name="${title}"><b>Product name: </b>${title}</span><br>
                          <span class="id grey-text" data-id="${product_id}"><b>Item Id:</b> ${product_id}</span><br>
                          <span class="url hide-details" data-url=""></span>
                          <span class="actual-price hide-details" data-actual-price="${compare_price !== undefined ? currency_code + " " + compare_price : 0}">
                            ${currency_code + " " + compare_price}
                          </span>
                          <span class="compare-price grey-text" data-compare-price="${currency_code + " " + price}">
                            <b>Price:</b> ${currency_code + " " + price}
                          </span>
                        </div>
                      </li>`
                  );
